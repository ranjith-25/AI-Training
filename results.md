...
p50 latency per query (baseline) = **54.7 ms**
...
| **p50 latency / query** | **54.7 ms** | **86.2 ms** |
| delta | - | **+8.3 pp** / **+31.5 ms** |

The latency cost is +31.5 ms p50 — a 57.6% increase in wall-clock time per query. At sub-100 ms
...
hit-rate@3 moved from 91.7% → 100% (+8.3 pp) with a +31.5 ms p50 overhead (54.7 ms → 86.2 ms).
The absolute latency (86.2 ms p50) is still under 100 ms, well within the acceptable range
...
The latency increase would be worth investigating further if p50 crossed 200 ms; at 86.2 ms
